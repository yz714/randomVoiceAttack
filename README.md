...
- 📝 日志记录 - 完整的检测和播放日志
- 💾 数据持久化 - 自动保存噪声数据
- ⏰ 时间范围控制 - 可自定义图表显示的时间范围
- 📉 数据降采样 - 防止大量数据导致前端卡顿
- 🛡️ 稳定可靠 - 支持长时间7×24小时运行，所有goroutine带recover保护
...
### v0.9.0
- 新增时间范围控制功能（预设选项+自定义输入）
- 新增数据降采样（最多渲染500个点，防止前端卡顿）
- 分离图表刷新逻辑（小图表1秒，全局图表1分钟）
- 全局图表添加手动刷新按钮
- 配置化数据保留时间（天为单位，自动计算数据条数）
- 前端代码拆分为index.html、style.css、app.js三个独立文件
- 重构：优雅的goroutine recover封装（utils.Go()和utils.GoWithName()）
- 优化：页面布局调整，把时间范围控制作为第四个统计卡片
- 优化：前端请求频率降低，大幅减少数据传输量
- 修复：所有goroutine都加上recover保护，防止程序闪退
